...
| --- | --- | --- |
| Build tool | ~~`rollup-preset-solid` v3 via `rollup.config.js`~~ **`tsup-preset-solid` via `tsup.config.ts` (done, Stage 4, 2026-09-07)** | Hasn't published in ~1 year — stagnant, not dead, but the community has moved to **`tsup-preset-solid`** (solidjs-community, esbuild-based, actively maintained through 2025). It auto-generates the `exports` map (including the `"solid"` condition) instead of hand-maintaining it, and is what newer Solid map-wrapper libraries (e.g. `solid-maplibre`) now use. **Migrated in Stage 4** — see the Stage 4 checklist below for the two real bugs this surfaced (tsup's own package.json-dependency/`.type` introspection returning stale/empty reads at build time, worked around with an explicit `external`/`outExtension` in `tsup.config.ts`; and several pre-existing type errors across `MapGL`/`Camera`/`Draw/modes/multi_point.ts` that the old build's laxer declaration step never actually caught). |
| `package.json` `exports`/`"solid"` condition | Hand-written, matches the still-current pattern | Conceptually correct and required for Solid's compiler to see raw JSX in consumer apps. **Done (Stage 4)** — now generated by `tsup-preset-solid`'s `writePackageJson` on every `pnpm build`, from the single `src/index.tsx` entry (renamed from `.ts` — the preset only emits the `"solid"` condition for entries whose filename itself ends `.tsx`/`.jsx`). |
| Peer dependency ranges | ~~`"mapbox-gl": "*"`, `"solid-js": "*"`~~ **`"mapbox-gl": "^3.0.0"`, `"maplibre-gl": "^4.0.0 \|\| ^5.0.0 \|\| ^6.0.0"` (added), `"solid-js": "^1.8.0 <2.0.0"` (done, Stage 4, 2026-09-07)** | Too loose for a library whose components actively depend on version-specific APIs (`setConfigProperty` is v3+ only, WebGL2-only requirements, etc.). **Recommend pinning ranges**, e.g. `"mapbox-gl": "^3.0.0"`, `"maplibre-gl": "^4.0.0 || ^5.0.0"` (as an added, currently-missing explicit peer dep — right now MapLibre users satisfy the `mapbox-gl` peer dep with the `empty-npm-package` placeholder trick and there's no real acknowledgment of `maplibre-gl` as a legitimate peer in `package.json` at all), and `"solid-js": "^1.8.0 <2.0.0"` to fail loudly rather than silently break if Solid 2.0 ever becomes the resolved version. **Implemented range widened to `^4.0.0 || ^5.0.0 || ^6.0.0`** for `maplibre-gl` (not just `^4/^5` as originally drafted here) since `STAGE0_FINDINGS.md` later confirmed MapLibre's v6 transition had completed and nothing in Stages 0-3 found a v4-v6 compatibility gap beyond what `ctx.isMapLibre` already handles; both `mapbox-gl` and `maplibre-gl` are marked `optional` in `peerDependenciesMeta` since a consumer only ever really needs one (the other is satisfied via the `empty-npm-package` placeholder). **This is now also a prerequisite for the automated dependency updates below** — Dependabot cannot propose a version bump for a peer dependency range that already matches everything, so `mapbox-gl`/`maplibre-gl`/`solid-js` won't generate any update PRs at all until these ranges are tightened. **On why not keep `"*"` to preserve "always support the latest release":** the maintainer's original intent behind `"*"` was that a user should never be blocked from a new Mapbox/MapLibre/Solid release just because this library hadn't been manually bumped to acknowledge it yet — a reasonable goal, but `"*"` is the wrong mechanism for it, for two reasons discussed and confirmed 2026-09-07: (1) `"*"` can't distinguish "too new to have been tested yet" from "too old to work at all" — it equally permits Mapbox GL JS v1.x, which lacks several APIs this library now depends on outright; (2) peer-dependency range mismatches are non-fatal **warnings**, not install-blocking errors, under npm 7+/pnpm/yarn — a capped range like `^3.0.0` does not actually prevent a consumer from installing/running `mapbox-gl@4`, it just surfaces an honest warning instead of silence. Combined with the weekly Dependabot automation, a capped range gets the *better* version of the original goal: a proposed, CI-tested bump shows up automatically within days of a new major shipping (flagged for manual review, per the maintainer's own choice on major-bump handling), rather than "*"'s actual historical track record — years of silent incompatibility discovered only via user bug reports, which is the exact problem this whole audit exists to fix. **Decision: use capped, lower-bounded ranges (not `"*"`, not unbounded-lower-only `>=3.0.0`)** — an unbounded-lower-only range would reintroduce the same blind spot as `"*"` for *future* majors specifically, since Dependabot has nothing to propose when there's no upper bound to bump. |
...
| Turf.js dependency | `@turf/area`, `@turf/center-of-mass`, `@turf/centroid`, `@turf/length`, `@turf/midpoint` as hard `dependencies` (not peer/dev) | **Revisited, no change (Stage 4, 2026-09-07).** 3.2 was resolved in Stage 1 as "finish wiring in" (not drop) — confirmed still genuinely wired in: `Draw/index.tsx` imports and registers `radius`/`line_string`/`polygon`/`rectangle`'s modes, each of which calls `getLength`/`getArea` from `modes/measurements.ts` for live length/area labels. Since the feature was kept, these 5 packages stay as hard dependencies exactly as this row originally said they should if kept — no dependency change needed. |
...
- ~~Migrate `rollup.config.js`/`rollup-preset-solid` → `tsup-preset-solid`, letting it regenerate
  the `exports` map~~ — **done.** `tsup.config.ts` replaces `rollup.config.js`; the library entry
  moved from `src/index.ts` to `src/index.tsx` (a pure rename, no JSX added — required for
  `tsup-preset-solid` to emit the `"solid"` export condition, which it gates on the entry
  filename's own extension, not whether the file contains JSX). Two build-breaking issues surfaced
  and were worked around directly in `tsup.config.ts`, both traced to the same root cause (tsup's
  own `getProductionDeps`/`pkg.type` package.json reads returning stale/empty data at build time
  regardless of the file's actual contents): (1) without an explicit `external` list, `mapbox-gl`/
  `maplibre-gl`/`three`/`@babylonjs/core` were silently bundled straight into `dist/` (multi-MB
  chunks) instead of staying external per their peer-dependency status; (2) without an explicit
  `outExtension` override, the non-`"solid"` build emitted `dist/index.mjs` while
  `writePackageJson`'s own output unconditionally pointed `package.json`'s `main`/`module` at
  `dist/index.js`. Also surfaced (and fixed, since they now hard-fail `pnpm build`'s dts step
  where the old rollup-based build silently tolerated them): `Draw/modes/measurements.ts`'s
  `getLength`/`getArea` inferred a non-portable `@types/geojson` return type (now explicit `any`
  — these are module-private, never exported); `MapGL`/`Camera`'s custom `evt.rotate`/
  `evt.clickOnLayer`/`event.viewport` properties (attached by this library's own event-emission
  code, not part of mapbox-gl's real event types — a latent gap since Stage 2 replaced the deep
  `mapbox-gl/src/...` type imports) needed explicit `as any` casts at each access point, matching
  this file's existing cast conventions; `Draw/modes/multi_point.ts`'s untyped `const
  MultiPointMode = {}` needed an explicit `any` annotation for the same reason. `mapbox-gl.css`'s
  import (pulled in by `MapGL`) is now left as an external, passed-through `import` statement
  instead of being snapshotted into solid-map-gl's own bundled CSS output — see `MIGRATION.md`'s
  "Non-breaking but worth knowing about" section.
- ~~Tighten peer dependency ranges (`mapbox-gl`, add explicit `maplibre-gl`, `solid-js`)~~ —
  **done.** See the Section 6 table row above for the exact ranges and the `peerDependenciesMeta`
  reasoning (both `mapbox-gl`/`maplibre-gl` marked optional).
- ~~Revisit whether Turf.js belongs as a hard dependency~~ — **done, no change.** See the Section 6
  table row above — confirmed still genuinely wired in, stays a hard dependency.
- ~~14.2 Add TypeDoc (+ `typedoc-plugin-markdown`)~~ — **done.** `typedoc.json` (entry
  `src/index.tsx`, output `docs/api/`) + `tsconfig.typedoc.json` (extends the root tsconfig with
  `types: []`, isolating the API-doc program from ambient test-tooling globals like
  `@testing-library/jest-dom`'s `expect` augmentation, which otherwise leak in and collide with
  `lib.dom.d.ts`). Root `tsconfig.json` also gained `"skipLibCheck": true` (a genuinely missing,
  effectively-universal setting — without it, a pre-existing, unrelated TS 5.9.3 `lib.dom.d.ts`
  internal inconsistency around `HTMLCanvasElement.requestPointerLock` blocks any full-program type
  check, including this one) and a corrected `exclude` list (the old
  `"**/__tests__/*"` pattern never matched this repo's actual `*.test.tsx` naming, so test files
  and `src/vitest.ts` were always accidentally in scope for anything reading this tsconfig — dead
  weight for `tsup`'s dts build specifically, since it only walks files reachable from
  `src/index.tsx`, but a real problem for TypeDoc/`tsc`, which type-check the whole `include` set
  regardless of reachability). `pnpm docs:api` generates Markdown into `docs/api/` (committed);
  `SUMMARY.md` links it in for GitBook's Git Sync. Independent of Decision B (Section 9 item 8,
  still open) on whether to migrate off GitBook.
- ~~14.3 Move the ~25 live examples from separately-hosted StackBlitz projects into an `examples/`
  directory~~ — **done**, for all 31 examples currently listed in `docs/examples.md` (two
  additional ones — `sync`/`swipe` — that the original "~25" estimate undercounted, plus the doc's
  actual full count across all 5 tabs). Each is a standalone Vite+Solid+TS app under
  `examples/<slug>/` depending on the *published* `solid-map-gl` (not a local/workspace link) —
  see `examples/README.md`. `docs/examples.md`'s links now point at
  `stackblitz.com/fork/github/gishub4/solid-map-gl/tree/main/examples/<slug>` instead of
  hand-created StackBlitz projects. Not part of the pnpm workspace (`pnpm-workspace.yaml` has no
  `packages:` glob) and not built/tested/typechecked by anything at the repo root.
